...
ClipboardAssistant::ClipboardAssistant(QWidget *parent) : QWidget(parent), ui(new Ui::ClipboardAssistantClass) {
    ...
    connect(m_progressTimer, &QTimer::timeout, this, &ClipboardAssistant::showProgressDialog);
...
void ClipboardAssistant::showEvent(QShowEvent *e) {
...
void ClipboardAssistant::onRunActionSet(IClipboardModule*, QString asid) {
    ...
    closeProgressReporter();
    if (!isVisible()) {
        m_progressTimer->start(1000); // Show progress dialog after 1 second if still hidden
    }
...
void ClipboardAssistant::onBtnCancelClicked() { 
    if (m_currentExecutor) m_currentExecutor->stop();
    if (m_activeModule) { 
        m_activeModule->abort(); 
        ui->btnCancel->setVisible(false); 
        ui->labelStatus->setText(tr("Cancelled.")); 
        ui->progressBar->setVisible(false); 
        ui->textOutput->append(tr("\n[Cancelled]")); 
        m_activeModule = nullptr; 
    } 
    closeProgressReporter();
}

void ClipboardAssistant::showProgressDialog() {
    if (m_progressDlg || isVisible()) return;
    m_progressDlg = new QProgressDialog(tr("Processing..."), tr("Cancel"), 0, 0, this);
    m_progressDlg->setWindowTitle(tr("Clipboard Assistant"));
    m_progressDlg->setWindowModality(Qt::NonModal);
    m_progressDlg->setMinimumDuration(0);
    connect(m_progressDlg, &QProgressDialog::canceled, this, &ClipboardAssistant::onBtnCancelClicked);
    m_progressDlg->show();
}

void ClipboardAssistant::closeProgressReporter() {
...
void ClipboardAssistant::handleModuleOutput(const QString& t, bool a, bool f) { 
    ...
    if (m_progressDlg) {
        m_progressDlg->setLabelText(ui->labelStatus->text());
    }
